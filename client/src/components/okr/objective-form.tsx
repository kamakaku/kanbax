...
export function ObjectiveForm({ onSuccess }: ObjectiveFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof objectiveFormSchema>) {
    ...
    try {
      ...
      try {
        const newCycle = await apiRequest<{ id: number }>("/api/okr-cycles", "POST", newCyclePayload);
        ...
        const newObjective = await apiRequest<{ id: number }>("/api/objectives", "POST", payload);
