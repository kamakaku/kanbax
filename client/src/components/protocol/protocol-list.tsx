...
export function ProtocolList({ teamId, projectId, objectiveId }: ProtocolListProps) {
  ...
  const [editingNotes, setEditingNotes] = useState<{protocolId: number, itemId: string, content: string} | null>(null);
                        ...
                        <div key={item.id} className="bg-muted/50 p-3 rounded border">
                          ...
                          {(item.notes || item.richNotes) && (
                            <div className="mt-2">
                              ...
                              {editingNotes && editingNotes.protocolId === protocol.id && editingNotes.itemId === item.id ? (
                                <div className="mt-2 border rounded-md p-2">
                                  <RichTextEditor
                                    content={editingNotes.content}
                                    onChange={(content) => setEditingNotes({...editingNotes, content})}
                                        ...
                                        const updatedProtocol = {
                                          ...
                                          agendaItems: protocol.agendaItems?.map((agendaItem: any) => {
                                            ...
                                            if (agendaItem.id === item.id) {
                                              return {
                                                ...agendaItem,
                                                richNotes: editingNotes.content,
                                ...
                                item.richNotes ? (
                                  <div 
                                    className="text-sm prose prose-sm max-w-none mt-2 cursor-pointer hover:bg-muted/50 p-2 rounded-md transition-colors" 
                                    ...
                                    onClick={(e) => {
                                      ...
                                      setEditingNotes({
                                        ...
                                        itemId: item.id,
                                        content: item.richNotes || ""
                                      });
                                    }}
                                  />
                                ) : (
                                  <p 
                                    className="text-sm whitespace-pre-line mt-2 cursor-pointer hover:bg-muted/50 p-2 rounded-md transition-colors"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setEditingNotes({
                                        protocolId: protocol.id,
