...
export default function MyTasks() {
  ...
  const { data: tasks = [], isLoading, error } = useQuery<TaskWithDetails[]>({
    ...
    queryFn: async () => {
      ...
      // Sende Anfrage gleich beim ersten Laden - ohne Delay
      console.log("Sending GET request to /api/user-tasks", null);
      const result = await apiRequest<TaskWithDetails[]>("GET", "/api/user-tasks");

      // Erfolgsmeldungen für Debugging
      console.log("Response from /api/user-tasks:", 200, "OK");
      console.log("JSON response from /api/user-tasks:", result);
    ...
    staleTime: 0, // Keine Cache-Zeit, immer sofort neu laden
    refetchOnMount: true, // Beim erneuten Mounten der Komponente neu laden
    refetchOnWindowFocus: true, // Beim erneuten Fokus auf dem Fenster neu laden
    retry: 1, // Sofort einen Retry versuchen, falls die erste Anfrage fehlschlägt
    retryDelay: 500 // Retry nach 500ms
  });
  
  // Sofort beim ersten Laden einen manuellen Abruf erzwingen
  useEffect(() => {
    // Sofort nach dem Laden der Komponente die Aufgaben abrufen
    const fetchTasksManually = async () => {
      try {
        // Nur ausführen, wenn wir Zugriff haben
        if (hasTaskAccess) {
          console.log("Manueller erster Abruf der Aufgaben");
          await queryClient.invalidateQueries({ queryKey: ["/api/user-tasks"] });
          await queryClient.refetchQueries({ queryKey: ["/api/user-tasks"] });
        }
      } catch (error) {
        console.error("Fehler beim manuellen Abruf:", error);
      }
    };
    
    fetchTasksManually();
  }, [hasTaskAccess, queryClient]);
  ...
  const updateTaskMutation = useMutation({
    mutationFn: async (updatedTask: Task) => {
      console.log("API-Anfrage zur Aktualisierung mit:", updatedTask);
      
      // Überprüfen ob die Aufgabe ein persönlicher Task ist (ohne boardId)
      if (updatedTask.boardId === null || updatedTask.isPersonal) {
        // Für persönliche Aufgaben verwenden wir einen anderen Endpunkt
        return apiRequest("PATCH", `/api/user/tasks/${updatedTask.id}`, updatedTask);
      } else {
        // Standard-Aufgaben-Endpunkt
    ...
    onSuccess: (data) => {
      ...
      // Sofort mehrere Abfragen invalidieren für eine bessere Synchronisation
    ...
    onError: (error) => {
      console.error("Fehler bei Aufgaben-Update:", error);
      toast({
        title: "Fehler",
      ...
      // Bei Fehler den Cache zurücksetzen/aktualisieren
      queryClient.invalidateQueries({ queryKey: ["api/user/tasks/assigned"] });
      queryClient.invalidateQueries({ queryKey: ["/api/user-tasks"] });
      queryClient.invalidateQueries({ queryKey: ["/api/tasks"] });
    }
  ...
  const handleTaskUpdate = async (updatedTask: Task): Promise<void> => {
    // Stellen wir sicher, dass Arrays korrekt formatiert sind
    const task = {
      ...updatedTask,
      // Stellen sicher, dass Arrays als Arrays gesendet werden
      assignedUserIds: Array.isArray(updatedTask.assignedUserIds) ? updatedTask.assignedUserIds : 
                      (updatedTask.assignedUserIds ? [updatedTask.assignedUserIds] : []),
      labels: Array.isArray(updatedTask.labels) ? updatedTask.labels : 
              (updatedTask.labels ? [updatedTask.labels] : []),
      // Wir müssen sicherstellen, dass die Checklist-Items korrekt formatiert sind
    ...
    console.log("Aktualisiere Task mit Daten:", task);
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      // Setze die aktualisierte Daten sofort in den QueryClient-Cache, damit UI sofort aktualisiert wird
      queryClient.setQueryData(["/api/user-tasks"], updatedTasks);

      // Stellen sicher, dass Arrays korrekt formatiert sind
      const task = {
        ...draggedTask,
        status: destination.droppableId,
        order: destination.index,
        // Stellen sicher, dass Arrays als Arrays gesendet werden
        assignedUserIds: Array.isArray(draggedTask.assignedUserIds) ? draggedTask.assignedUserIds : 
                       (draggedTask.assignedUserIds ? [draggedTask.assignedUserIds] : []),
        labels: Array.isArray(draggedTask.labels) ? draggedTask.labels : 
               (draggedTask.labels ? [draggedTask.labels] : []),
        checklist: Array.isArray(draggedTask.checklist) ? draggedTask.checklist : [],
        attachments: Array.isArray(draggedTask.attachments) ? draggedTask.attachments : []
      };
          ...
          <div className="relative p-8">
            ...
            {viewMode === 'kanban' ? (
              <DragDropContext onDragEnd={handleDragEnd}>
                <div className="flex overflow-x-auto gap-6 mt-4 pb-4" style={{ minWidth: '100%' }}>
