...
export default function ProjectDetail() {
  ...
  const [limitWarningType, setLimitWarningType] = useState<"board" | "project" | "task" | "team" | "okr" | "user">("board");
  ...
  return (
    <div className="container py-6 space-y-6">
                  ...
                  onClick={async () => {
                    try {
                      ...
                      if (data.hasReachedLimit) {
                        ...
                        setLimitWarningType("board");
                        setShowLimitWarning(true);
                      } else {
                        // Navigiere zur Board-Erstellungsseite, wenn Limit nicht erreicht
                        navigate(`/boards/new?projectId=${projectId}`);
                      }
                    } catch (error) {
                      console.error("Fehler bei der Überprüfung des Board-Limits:", error);
                      // Bei Fehler trotzdem zur Board-Erstellungsseite navigieren
                      navigate(`/boards/new?projectId=${projectId}`);
                    }
                  }}
                >
      ...
      {/* Limit-Warnung bei Überschreitung der Abonnementgrenze */}
      <GenericLimitWarningDialog
        open={showLimitWarning}
        onOpenChange={setShowLimitWarning}
        title={
          limitWarningType === "board" ? "Board-Limit erreicht" :
          limitWarningType === "okr" ? "OKR-Limit erreicht" :
          limitWarningType === "task" ? "Aufgaben-Limit erreicht" :
          limitWarningType === "team" ? "Team-Limit erreicht" :
          limitWarningType === "user" ? "Benutzer-Limit erreicht" :
          "Ressourcen-Limit erreicht"
        }
        limitType={limitWarningType}
        resourceName={
          limitWarningType === "board" ? "Board" :
          limitWarningType === "okr" ? "OKR" :
          limitWarningType === "task" ? "Aufgabe" :
          limitWarningType === "team" ? "Team" :
          limitWarningType === "user" ? "Benutzer" :
          limitWarningType === "project" ? "Projekt" :
          "Ressource"
        }
        resourceNamePlural={
          limitWarningType === "board" ? "Boards" :
          limitWarningType === "okr" ? "OKRs" :
          limitWarningType === "task" ? "Aufgaben" :
          limitWarningType === "team" ? "Teams" :
          limitWarningType === "user" ? "Benutzer" :
          limitWarningType === "project" ? "Projekte" :
          "Ressourcen"
        }
        endpoint={`/api/subscription/check-limit/${limitWarningType}s`}
      />
    </div>
  );
}