...
app.get('/boot', async (req, res) => {
    ...
    const boardList = boards.length ? boards : [await ensureDefaultBoard(tenantId)];
    const activeBoardId = user?.preferences?.ui?.activeBoardByTenant?.[tenantId]
        || boardList[0]?.id
        || 'default-board';
    const membership = memberships.find((m) => m.tenantId === tenantId) || null;
    const role = user.isSuperAdmin ? 'SUPERADMIN' : (membership?.role || 'MEMBER');
    const principal = buildPrincipal(tenantId, user, role);
    const taskBoardId = activeBoardId === 'all' || activeBoardId === 'mine' || activeBoardId === 'archived'
        ? 'all'
        : activeBoardId;
...
app.put('/teams/:tenantId/scopes', async (req, res) => {
    ...
    try {
        ...
        const toUpsert = normalized.map((scope) => {
            ...
            const existingTaskIds = Array.isArray(existing.task_ids)
                ? existing.task_ids
        ...
        const memberUpdates = toUpsert.flatMap((scope) => {
            ...
            if (!canManage || !Array.isArray(scope.members))
                return [];
            ...
            return [
                ...
                `),
                ...withCreator.map((member) => prisma.$executeRaw(Prisma.sql `
    ...
    await prisma.$transaction([
        prisma.$executeRaw(Prisma.sql `
            ...
            where tenant_id = ${tenantId}
              and creator_id = ${user.id}
        `),
        ...payload.map((item) => prisma.$executeRaw(Prisma.sql `
...
app.put('/teams/:tenantId/timeline-overrides', async (req, res) => {
    ...
    if (taskIds.length === 0) {
        await prisma.$executeRaw(Prisma.sql `delete from huddle_timeline_overrides where tenant_id = ${tenantId}`);
