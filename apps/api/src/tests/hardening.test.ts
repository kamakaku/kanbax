...
import { CreateTaskPipeline, CreateTaskCommand } from '../create-task-pipeline';
...
async function runHardeningTests() {
    ...
    const manualCommand: CreateTaskCommand = {
        type: 'TASK_CREATE',
        tenantId: 'tenant-1',
    ...
    const jiraCommand: CreateTaskCommand = {
        type: 'TASK_CREATE',
        tenantId: 'tenant-1',
        principal: { id: 'user-1', tenantId: 'tenant-1', type: PrincipalType.USER, roles: [], metadata: {} },
        payload: {
    ...
    const deniedEvent = events.find((e: any) => e.action === AuditAction.ACCESS_DENIED);
    if (deniedEvent) {
    ...
    try {
        await repository.save({ id: 'invalid' } as any);
