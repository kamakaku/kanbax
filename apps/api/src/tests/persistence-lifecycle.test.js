...
class MockPrismaClient {
    ...
    task = {
        ...
        upsert: async ({ where, create, update }) => {
            ...
            if (existing && existing.tenantId !== where.tenantId)
        ...
        findMany: async ({ where }) => {
            return Array.from(this.data.task.values()).filter((t) => {
...
class TaskRepositoryPostgres {
    ...
    async findById(id, tenantId) { return this.prisma.task.findUnique({ where: { id, tenantId } }); }
    async save(task) { return this.prisma.task.upsert({ where: { id: task.id, tenantId: task.tenantId }, create: task, update: task }); }
}
class AuditEventRepositoryPostgres {
    ...
    async log(event) { return this.prisma.auditEvent.create({ data: event }); }
    async deleteBefore(date, tenantId) { return this.prisma.auditEvent.deleteMany({ where: { tenantId, timestamp: { lt: date } } }); }
}
// --- RETENTION SERVICE ---
class RetentionService {
    ...
    async runRetention(tenantId, retentionDays) {
...
async function runTests() {
    ...
    if (!task)
        console.log('Result: SUCCESS (Tenant B cannot read Tenant A data)');
    else
...
//# sourceMappingURL=persistence-lifecycle.test.js.map